...
class VVS:
    ...
    def undistort_image(self, K, dist, img):
        h,  w = img.shape[:2]
        new_K, roi = cv2.getOptimalNewCameraMatrix(K, dist, (w,h), 1, (w,h))
        x, y, w, h = roi
        undistorted_img = cv2.undistort(img, K, dist, None, new_K)[y:y+h, x:x+w]
    ...
    def get_disparity_map(self, F, left_img, right_img):
        ...
        disparity_map = np.zeros((h, tensor_depth))
        pixel_indexes = np.array(range(tensor_depth))
        end = h - f_size -1
        end = 367
        for start_h in range(0, end):
